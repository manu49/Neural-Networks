...
for x in train_data_temp:
	...
	text = x['text']
	#print(type(text))
	words_list = _stem(text,p_stemmer,en_stop,True)
...
for x in train_data:
	if(i<100):
		...
		for t in _stem(text,p_stemmer,en_stop):
...
file1.write("Accuracy is : "+str(check_accuracy(Labels_test,predictions))+"\n")
file1.close()
#file2.close()
...
print(_stem(new_text,p_stemmer,en_stop,True))'''